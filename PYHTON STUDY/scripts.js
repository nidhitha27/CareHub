...
function sendOtp() {
    ...
    const isValidMobileNumber = /^\d+$/.test(mobileNumber);

    if (!isValidMobileNumber) {
        alert("Please enter only numeric values in the mobile number field.");
        return;
    }
...
function startDiabetesNotifications() {
    ...
    }, 5000); // Sends notification every hour (3600000 milliseconds)
    setInterval(() => {
